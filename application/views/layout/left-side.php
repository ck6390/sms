<div class="col-md-3 <?php echo $this->gsms_setting->enable_rtl ? 'right_col' : 'left_col'; ?>">
    <div class="<?php echo $this->gsms_setting->enable_rtl ? 'right_col' : 'left_col'; ?> scroll-view">
        <div class="navbar nav_title text-center py-5" style="border: 0;">
            <a href="<?php echo site_url('dashboard'); ?>" class="site_title_">
            <?php if($this->session->userdata('school_logo')){?>               
                <img class="img-thumbnail img-circle logo" src="<?php echo $this->config->item('base_url'); ?><?php echo '/assets/logo/'.$this->session->userdata('school_logo'); ?>" alt="" width="80" />
        ...
        <div class="text-center">                
            <?php 
			 if($this->session->userdata('user_type') == "School"){
                ...
                $days = "days";
                $expiry = date_create($this->session->userdata('expiry_date'));
                $current_date = date_create(date('d-m-Y'));		
                $diff = date_diff($current_date,$expiry);                
				$interval = $diff->format("%a");
                if($interval <= 10 )
                {
                     echo "<p class='btn btn-danger btn-xs '>".$notification." ".$interval." ".$days."</p>";
                }
        ...
        <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
            <div class="menu_section">
                <?php   
                 if(empty($this->session->userdata('alloted_menu_id'))){
                    echo "<div class='col-md-12'>
                            <p class='text-center text-danger'>Your not alloted any module. Please contact to service provider.</p>
                        </div>";
                 }
                 else{               
                 $alloted_menu_id = explode(",", $this->session->userdata('alloted_menu_id')); 
				 //var_dump($alloted_menu_id);
				// die();                
                    $this->load->model('Dashboard_Model', 'dashboard', true);
                    $classes = $this->dashboard->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');                
                ?>
                <ul class="nav side-menu"> 
                    <?php 
                        foreach ($this->session->userdata('all_menu') as $val) {
                            $menu_id[$val->id]=$val->id;
                        } 
                        foreach ($alloted_menu_id as $obj) {
                        if(@$menu_id[$obj] == "1") {                   
                    ?>          
                    <li><a href="<?php echo site_url('dashboard'); ?>"><i class="fa fa-dashboard"></i> <?php echo $this->lang->line('dashboard'); ?></a>  </li>
                    <?php } elseif(@$menu_id[$obj] == "2"){?>
                        ...
                        <li><a><i class="fa fa-user"></i> <?php echo $this->lang->line('administrator'); ?> <span class="fa fa-chevron-down"></span></a>
                            <ul class="nav child_menu">
                                ...
                                <?php if($this->session->userdata('role_id') == 1){ ?>   
                                    <li><a href="<?php echo site_url('student/bulk/add/'); ?>"> Student <?php echo $this->lang->line('import'); ?></a></li>                         
                                <?php } ?>
                                <?php if($this->session->userdata('role_id') == 1){ ?>   
                                    <li><a href="<?php echo site_url('teacher/bulk/add/'); ?>">Teacher <?php echo $this->lang->line('import'); ?></a></li>                         
                                <?php } ?>
                            </ul>
                        </li>
                    <?php } } elseif(@$menu_id[$obj] == "4") { ?>                                    
                    ...
                    <?php if(has_permission(VIEW, 'hrm', 'designation') || has_permission(VIEW, 'hrm', 'employee')){ ?>    
                            ...
                            <ul class="nav child_menu">
                                ...
                                <?php } ?>
                                <?php if(has_permission(VIEW, 'hrm', 'leave')){ ?>   
                                    <li><a href="<?php echo site_url('hrm/leave'); ?>"><?php echo $this->lang->line('manage_leave'); ?></a></li>
                                <?php } ?>
                                <?php if(has_permission(VIEW, 'hrm', 'employeeleave')){ ?>   
                                    <li><a href="<?php echo site_url('hrm/employeeleave'); ?>"><?php echo $this->lang->line('manage_employee_leave'); ?></a></li>
                                <?php } ?>
                            </ul>
                        </li> 
                    <?php }}  elseif(@$menu_id[$obj] == "14"){?>
                    ...
                    <?php } }  elseif(@$menu_id[$obj] == "15"){?>                        
                        
                    <?php if(has_permission(VIEW, 'payroll', 'grade') || has_permission(VIEW, 'payroll', 'payment')){ ?>
                        <li><a><i class="fa fa-inr"></i> <?php echo $this->lang->line('payroll'); ?> <span class="fa fa-chevron-down"></span></a>
                            <ul class="nav child_menu">
                                <?php if(has_permission(VIEW, 'payroll', 'grade')){ ?>  
                                    <li><a href="<?php echo site_url('payroll/grade/index'); ?>"><?php echo $this->lang->line('salary_grade'); ?></a></li>
                                <?php } ?>
                        ...
                        <li><a <?php if(empty($classes)){ ?> onclick="alert('<?php echo $this->lang->line('class_add_alert'); ?>');" <?php } ?>><i class="fa fa-thumb-tack"></i><?php echo $this->lang->line('exam'); ?> <?php echo $this->lang->line('schedule'); ?> <span class="fa fa-chevron-down"></span></a>
                            <ul class="nav child_menu">                                                          
                                <?php foreach($classes as $obj){ ?>
                                    ...
                                    <?php } ?>                            
                                <?php } ?>   
                                <li><a href="<?= site_url('exam/examhallticket/view/') ?>"><?= $this->lang->line('exam_hall_ticket'); ?></a></li>                         
                            </ul>
                        </li> 
                    <?php } } elseif(@$menu_id[$obj] == "27"){?>   
                    ...
                    <?php if(has_permission(VIEW, 'report', 'report')){ ?>
                        <li><a><i class="fa fa-bar-chart"></i> <?php echo $this->lang->line('report'); ?> <span class="fa fa-chevron-down"></span></a>
                            <ul class="nav child_menu">
                                ...
                                <li><a href="<?php echo site_url('report/balance'); ?>"><?php echo $this->lang->line('accounting'); ?> <?php echo $this->lang->line('balance'); ?> <?php echo $this->lang->line('report'); ?></a></li> <li><a href="<?php echo site_url('report/accountant'); ?>"><?php echo $this->lang->line('accountant'); ?> <?php echo $this->lang->line('report'); ?></a></li>
                   ...
                   <!--?php if(has_permission(VIEW, 'gallery', 'gallery') || has_permission(VIEW, 'gallery', 'image')){ ?>     
                    ...
                    <li><a><i class="fa fa-lock"></i><?php echo $this->lang->line('profile'); ?> <span class="fa fa-chevron-down"></span></a>
                        <ul class="nav child_menu">
                            <li><a href="<?php echo site_url('setting'); ?>"><?php echo $this->lang->line('my_profile'); ?></a></li>
