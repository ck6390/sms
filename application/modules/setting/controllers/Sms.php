...
class Sms extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->data['setting'] = $this->setting->get_single('sms_settings', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
    }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($this->input->post('id')) {
                    $update = $this->setting->update('sms_settings', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
                    if ($update) {
    ...
    * ********************************************************** */
    private function _prepare_smscountry_validation() {
        ...
        /*****************Function msgclub**********************************
    * @type            : Function
    * @function name   : msgclub
    * @description     : Load "Msg91 Setting Tab" user interface                 
    *                     and process to save Msg91 setting inormation into database  
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function msgclub() {

        check_permission(EDIT);

        if ($_POST) {
            $this->_prepare_msgclub_validation();
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($this->input->post('id')) {
                    ...
                    if ($update) {
                        ...
                        create_log('Has been updated msg club SMS gateway setting');
                        success($this->lang->line('update_success'));
                    } else {
                        error($this->lang->line('update_failed'));
                    }
                    ...
                    if ($insert_id) {
                        ...
                        create_log('Has been added msg club SMS gateway setting');
                        success($this->lang->line('insert_success'));
                    } else {
                        error($this->lang->line('insert_failed'));
                    }
                }
                redirect('setting/sms/msgclub');
            } else {
                $this->data = $_POST;
            }
        }

        $this->data['msgclub'] = TRUE;
        $this->layout->title($this->lang->line('sms') . ' ' . $this->lang->line('setting') . ' | ' . SMS);
        $this->layout->view('sms/index', $this->data);
    }

                
    /*****************Function _prepare_msgclub_validation**********************************
    * @type            : Function
    * @function name   : _prepare_msgclub_validation
    * @description     : Process "Msg91 Gateway" user input data validation                 
    *                       
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    private function _prepare_msgclub_validation() {
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<div class="error-message" style="text-align: center;color: red;">', '</div>');

        $this->form_validation->set_rules('msgclub_auth_key', $this->lang->line('auth_key'), 'trim|required');
        $this->form_validation->set_rules('msgclub_sender_id', $this->lang->line('sender_id'), 'trim|required');
        $this->form_validation->set_rules('msgclub_status', $this->lang->line('is_active'), 'trim|required');
    }
    ...
    * ********************************************************** */
    private function _get_posted_sms_data() {
        ...
        if ($this->input->post('msgclub')) {
            $items[] = 'msgclub_auth_key';
            $items[] = 'msgclub_sender_id';
            $items[] = 'msgclub_status';
        }
