            ...
            <div class="x_content"> 
                ...
                <div class="row"> 
                    <div class="col-md-3 col-sm-3 col-xs-12 col-sm-offset-4">
                        <div class="item form-group"> <!--?php if(isset($date)){ echo $date;} ?--> 
                            <div><?php echo $this->lang->line('date'); ?> <span class="required">*</span></div>
                            <input  class="form-control col-md-7 col-xs-12"  name="date"  id="date" value="<?= date('d-m-Y')?>" placeholder="<?php echo $this->lang->line('date'); ?>" required="required" type="text" autocomplete="off" readonly>
                    ...
                    <div class="col-md-2"><br/>
                      <a href="<?= $this->config->item('att_url') ?>" target="_new" class="btn btn-primary btn-sm"><?= $this->lang->line('shrink_attandance') ?></a> 
                    </div>    
                </div>
                <?php echo form_close(); ?>
            </div>
                ...
                <div class="row">
                    <div class="col-sm-4  col-sm-offset-4 layout-box">
                        <p>
                            ...
                            <?php echo $this->lang->line('date'); ?> : <?php echo date($this->config->item('date_format'), strtotime($date)); ?>
                        </p>
                    </div>
                </div>            
            </div>
                    ...
                    <tbody id="fn_attendance">   
                        ...
                        if (isset($teachers) && !empty($teachers)) {
                            ...
                            <?php  $attendance = get_teacher_attendance($obj->id, $academic_year_id, $year, $month, $day); 
                               $json_data = json_decode($attendance->attendance_data);                              
                                $attendance = '';
                                foreach ($json_data as $var) {
                                  if($day == $var->day && $var->attendance == "P"){
                                    $attendance = $var->attendance;
                                  }elseif($day == $var->day && $var->attendance == "A"){
                                    $attendance = $var->attendance;
                                  }elseif($day == $var->day && $var->attendance == ""){
                                    $attendance = "A";
                                  }
                                }    
                            ?>
                        ...
                        <?php } } else{ ?>
                         <tr>
                          <td colspan="9" align="center"><h1 class="text-danger"><?php echo $holiday_msg; ?></h1></td>
                        </tr>
                        <?php } ?>
                    </tbody>
                </table>
