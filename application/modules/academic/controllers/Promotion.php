...
class Promotion extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->data['classes'] = $this->promotion->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['exams'] = $this->promotion->get_list('exams', array('status' => 1, 'academic_year_id' => $this->academic_year_id,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['grades'] = $this->promotion->get_list('grades', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['curr_session'] = $this->promotion->get_single('academic_years', array('is_running' => 1,'school_id'=> $this->session->userdata('school_id')));
        $this->data['next_session'] = $this->promotion->get_list('academic_years', array('id !=' => $this->academic_year_id, 'status'=>1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'session_year', 'ASC');
    ...
    public function index() {                
        ...
        if($_POST){
            ...
            $this->data['current_class'] = $this->promotion->get_single('classes', array('id' => $current_class_id,'school_id'=> $this->session->userdata('school_id')));
            $this->data['next_class'] = $this->promotion->get_single('classes', array('id' => $next_class_id,'school_id'=> $this->session->userdata('school_id')));
     ...
     * ********************************************************** */
     public function add() {                
        ...
        if($_POST){
           ...
           //var_dump($current_session_id);
           //die();
            // get next class default section
            $next_class_default_section = $this->promotion->get_single('sections', array('class_id'=>$next_class_id, 'name'=>'A','school_id'=> $this->session->userdata('school_id')));
            ...
            if(!empty($_POST['students'])){
                ...
                foreach($_POST['students'] as $key=>$value){
                    ...
                    if($_POST['promotion_class_id'][$value] == $current_session_id){
                    ...
                    // need to check is any student alredy enrolled
                     $exist = $this->promotion->get_single('enrollments', array('class_id'=>$current_class_id, 'student_id'=>$value, 'academic_year_id'=>$current_session_id,'school_id'=> $this->session->userdata('school_id')));
                     //var_dump($exist);
                    // die();
                     $data = array();
                     $data['class_id'] = $_POST['promotion_class_id'][$value]; 
                     $data['roll_no'] = $_POST['roll_no'][$value]; 
                     $data['academic_year_id'] = $this->input->post('next_session_id'); 
                    ...
                    if(empty($exist)){ 
                       ...
                       // die();
                        ...
                        $this->promotion->update('enrollments', $data, array('student_id'=>$value, 'academic_year_id'=>$current_session_id,'school_id'=> $this->session->userdata('school_id'))); 
                    }
                }
            }
