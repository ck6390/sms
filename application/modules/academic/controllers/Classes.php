...
class Classes extends MY_Controller {
    ...
    public function index() {
        ...
        $this->data['teachers'] = $this->classes->get_list('teachers', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
        $this->data['list'] = TRUE;
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) { 
                    ...
                    if(empty($this->input->post('section'))){
                        $section_id = $this->__create_default_section($insert_id);
                    }
                    $this->__create_selected_section($insert_id);
                    //Start for add Class from ajax
                    if($this->input->post('teacher_id') == 0){
                        echo json_encode(['class_id'=>$insert_id,'section_id'=>$section_id,'success'=>'Class added successfully']);
                        exit;
                    }
                    //End for add Class from ajax
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_class_data();
                $updated = $this->classes->update('classes', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
                ...
                $this->data['class'] = $this->classes->get_single('classes', array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
            }
        }
        
        if ($id) {
            $this->data['class'] = $this->classes->get_single('classes', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
   ...
   {             
    ...
    public function delete($id = null) {
        ...
        $class = $this->classes->get_single('classes', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
                
        if ($this->classes->delete('classes', array('id' => $id,'school_id'=> $this->session->userdata('school_id')))) { 
    ...
    private function __create_default_section($insert_id){
        ...
        $data['status']     = 1; 
        $data['school_id']  = $this->session->userdata('school_id'); 
        return $insert_id = $this->classes->insert('sections', $data);
    }

    /*****************Function __create_selected_section**********************************
     * @type            : Function
     * @function name   : __create_selected_section
     * @description     : create default section while create a new class
     *                       
     * @param           : $insert_id integer value
     * @return          : null 
     * ********************************************************** */
    private function __create_selected_section($insert_id){
        if(!empty($this->input->post('section'))){
            foreach ($this->input->post('section') as $section) {
                $data = array();
                $data['class_id']    = $insert_id;
                $data['teacher_id']  = $this->input->post('add_'.$section);
                $data['name']       = $section;
                $data['note']       = 'Default Selected Section';
                $data['created_at'] = date('Y-m-d H:i:s');
                $data['created_by'] = logged_in_user_id();
                $data['status']     = 1; 
                $data['school_id']  = $this->session->userdata('school_id'); 
                //return $insert_id = $this->classes->insert('sections', $data);
                $section = $this->section->duplicate_check($insert_id,$section); 
                if(empty($section)){
