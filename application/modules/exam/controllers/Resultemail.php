...
class Resultemail extends MY_Controller {
    ...
    * ********************************************************** */
    private function _send_email($data) {
        ...
        $receivers = '';
        $users = $this->mail->get_user_list($data['role_id'], $this->input->post('receiver_id'), $class_id);
        ...
        foreach ($users as $obj) {
            ...
            if($get_role_by_id->name == "Student"){
                ...
                $student = $this->mail->get_single_student($obj->guardian_id,  $class_id);                 
                $student_id = $student->student_id;
                $section_id = $student->section_id; 
            }            
            
            $result = $this->mail->get_single('final_results',array('student_id'=> $student_id, 'class_id'=>$class_id, 'academic_year_id'=> $this->academic_year_id));
            
            if(!empty($result)){
                
                $grade = $this->mail->get_single('grades',array('id'=> $result->grade_id));

                $class_position = get_position_student_position($this->academic_year_id, $class_id, $student_id);
                $section_position = get_position_student_position($this->academic_year_id, $class_id, $student_id, $section_id);
