...
class Mail extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->data['exams'] = $this->mark->get_list('exams', array('status' => 1, 'academic_year_id' => $this->academic_year_id,'school_id' => $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['roles'] = $this->mark->get_list('roles', array('status' => 1,'school_id' => $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    private function _send_email($data) {
        ...
        $students = $this->mark->get_student_list_by_class($data['role_id'], $data['exam_id'], $data['class_id'], $this->input->post('receiver_id'));
        $setting = $this->mark->get_single('settings', array('status' => 1,'school_id' => $this->session->userdata('school_id')));
        ...
        if (!empty($students)) {
            ...
            foreach ($students AS $obj) {
                ...
                $receiver = $this->mark->get_receiver_email($data['role_id'], $obj->student_id);
                ...
                if(!empty($marks)){
                    ...
                    $body = get_formatted_body($data['body'], $receiver->role_id, $receiver->id);
            ...
            // update emails table 
            $this->mark->update('mark_emails', array('receivers' => $receivers), array('id' => $data['email_id'],'school_id' => $this->session->userdata('school_id'))); 
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $email = $this->mark->get_single('mark_emails', array('id' => $id,'school_id' => $this->session->userdata('school_id')));
        if ($this->mark->delete('mark_emails', array('id' => $id,'school_id' => $this->session->userdata('school_id')))) {
