                ...
                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;" colspan="2"><?php echo $this->lang->line('mark').'s'; ?> 
                            </th>                                            
                            <th rowspan="2"><?php echo $this->lang->line('comment'); ?></th>                                            
                        </tr>
                        <?php if(@current($students)->subject_type != 'co-curricular'){?>
                        <tr>                                           
                            <th>
                                <?php echo $this->lang->line('total'); ?> 
                                <?php echo $this->lang->line('mark'); ?>
                            </th>                                            
                            <th>
                        ...
                        if (isset($students) && !empty($students)) {
                            ...
                            <?php  $attendance = get_exam_attendance($obj->student_id, $this->academic_year_id, $exam_id, $class_id, $section_id, $subject_id); ?>
                                <tr>
                                    ...
                                    <?php if($obj->subject_type != 'co-curricular'){?>
                                    <td>
                                        <input type="number"  id="exam_total_mark_<?php echo $obj->student_id; ?>" value="<?php if(!empty($mark)){ echo $mark->exam_total_mark; }else{ echo '';} ?>"  name="exam_total_mark[<?php echo $obj->student_id; ?>]" class="form-control form-mark col-md-7 col-xs-12"  />
                                    </td>
                                    <td>
                                        <?php if(!empty($attendance)){ ?>
                                            <input type="number" id="obtain_total_mark_<?php echo $obj->student_id; ?>" value="<?php if(!empty($mark)){ echo $mark->obtain_total_mark; }else{ echo ''; } ?>"  name="obtain_total_mark[<?php echo $obj->student_id; ?>]" class="form-control form-mark col-md-7 col-xs-12" />
                                        <?php }else{ ?>
                                    ...
                                    <?php }else{ ?>
                                    <td>
                                        <select name="grade_id[<?php echo $obj->student_id; ?>]" class="form-control col-md-7 col-xs-12" <?php //echo ($attendance)? '':'readonly="readonly"' ?> >
                                            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>
                                            <?php $types = get_grades(); ?>
                                            <?php foreach($types as $key=>$value){ ?>
                                                <option value="<?php echo $key; ?>" <?php if(isset($mark) && $mark->grade_id == $key){ echo 'selected="selected"';} ?>><?php echo $value; ?></option>
                                            <?php } ?>
                                        </select>
                                    </td>
                                    <?php }?>
                                    <td colspan="2">
                                        <?php //if(!empty($attendance)){ ?>
                                            <input type="text"  id="remark_<?php echo $obj->student_id; ?>" value="<?php if(!empty($mark) && $mark->remark != '' ){ echo $mark->remark; }else{ echo ''; } ?>"  name="remark[<?php echo $obj->student_id; ?>]" class="form-control form-mark col-md-7 col-xs-12" />
                                        <?php /*}else{ ?>
                                            <input readonly="readonly" type="text" value=""  name="remark[<?php echo $obj->student_id; ?>]" class="form-control form-mark col-md-7 col-xs-12"  />
                                        <?php } */?>
                                    </td>
                                </tr>
                            <?php } ?>
