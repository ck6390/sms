...
class Role extends MY_Controller {
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['roles'] = $this->role->get_list('roles', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
        $this->data['add'] = TRUE;
    ...
    public function edit($id = null) { 
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $this->data['roles'] = $this->role->get_list('roles', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
                $this->data['role']  = $this->role->get_single('roles', array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
            }
        } else {
            if ($id) {
    ...
    * ********************************************************** */
    private function _prepare_role_validation() {
        ...
        $this->form_validation->set_rules('name', $this->lang->line('role').' '.$this->lang->line('name'), 'trim|required|callback_name|is_unique[roles.name]');
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $role = $this->role->get_single('roles', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        
        if ($this->role->delete('roles', array('id' => $id,'school_id'=> $this->session->userdata('school_id')))) { 
