...
class Password extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
         ...
         $this->data['roles'] = $this->administrator->get_list('roles', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
         $this->data['years'] = $this->administrator->get_list('academic_years', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
    }
   ...
   public function index() {
       ...
       check_permission(EDIT);
        ...
        if($_POST){
            ...
            $this->form_validation->set_rules('conf_password', $this->lang->line('password').' '.$this->lang->line('confirm'), 'trim|required|matches[password]');
             ...
             if ($this->form_validation->run() === TRUE) {
                ...
                $this->administrator->update('users', $data, array('id'=> $this->input->post('user_id'),'school_id'=> $this->session->userdata('school_id')));
                success($this->lang->line('update_success'));
                
                $user = $this->administrator->get_single('users', array('id' => $this->input->post('user_id'),'school_id'=> $this->session->userdata('school_id')));
        ...
        $this->data['classes'] = $this->administrator->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
