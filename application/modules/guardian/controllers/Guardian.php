...
class Guardian extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['roles'] = $this->guardian->get_list('roles', array('status' => 1,'school_id' => $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_guardian_data();
                $updated = $this->guardian->update('guardians', $data, array('id' => $this->input->post('id'),'school_id' => $this->session->userdata('school_id')));
       ...
       // $items[] = 'modal_data';
        ...
        if ($this->input->post('id')) {
            ...
            if(!empty($this->input->post('email'))){
                ...
                $users = $this->guardian->get_single('users', array('email' => $email), array('id','email'));
                if($users->email != $email){
                    $updated = $this->guardian->update('users', array('email' => $email), array('id' => $this->input->post('uid'),'school_id' => $this->session->userdata('school_id')));
                }
            }
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $guardian = $this->guardian->get_single('guardians', array('id' => $id,'school_id' => $this->session->userdata('school_id')));
        if (!empty($guardian)) {

            // delete guardian data
            $this->guardian->delete('guardians', array('id' => $id,'school_id' => $this->session->userdata('school_id')));
            // delete guardian login data
            $this->guardian->delete('users', array('id' => $guardian->user_id,'school_id' => $this->session->userdata('school_id')));
    ...
    * ***********************************************************/
    ...
    public function deactivate($id)
    {
        ...
        if ($this->guardian->deactivateUser($id))
        {
            # Add success
            success('Updated status.(Deactivated');
            redirect('guardian', 'referesh');
        }
        else
        {
            # Failed
    ...
    public function activate($id)
    {
        ...
        if ($this->guardian->activateUser($id))
        {
