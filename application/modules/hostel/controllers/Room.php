...
class Room extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['hostels'] = $this->room->get_list('hostels', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    $hostel = $this->room->get_single('hostels', array('id' => $data['hostel_id'],'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_room_data();
                $updated = $this->room->update('rooms', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
                ...
                $this->data['room'] = $this->room->get_single('rooms', array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
            }
        }

        if ($id) {
            $this->data['room'] = $this->room->get_single('rooms', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $room = $this->room->get_single('rooms', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        
        if ($this->room->delete('rooms', array('id' => $id))) {
            
            $hostel = $this->room->get_single('hostels', array('id' => $room->hostel_id,'school_id'=> $this->session->userdata('school_id')));
