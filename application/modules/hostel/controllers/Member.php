...
class Member extends MY_Controller {
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['hostels'] = $this->member->get_list('hostels', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
        $this->data['add'] = TRUE;
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $member = $this->member->get_single('hostel_members', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        if ($this->member->delete('hostel_members', array('id' => $id))) {

            $this->member->update('students', array('is_hostel_member' => 0), array('user_id' => $member->user_id,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function add_to_hostel() {
        ...
        if ($user_id) {
            ...
            $member = $this->member->get_single('hostel_members', array('user_id' => $user_id,'school_id'=> $this->session->userdata('school_id')));
            if (empty($member)) {
                ...
                $this->member->update('students', array('is_hostel_member' => 1), array('user_id' => $user_id,'school_id'=> $this->session->userdata('school_id')));
                
                $member = $this->member->get_single('students', array('user_id' => $user_id,'school_id'=> $this->session->userdata('school_id')));
