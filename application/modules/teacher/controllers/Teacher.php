...
class Teacher extends MY_Controller {
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_teacher_data();
                $updated = $this->teacher->update('teachers', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    private function _prepare_teacher_validation() {
        ...
        $this->form_validation->set_rules('phone', $this->lang->line('phone'), 'trim|required|min_length[10]');
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $teacher = $this->teacher->get_single('teachers', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        if (!empty($teacher)) {

            // delete teacher data
            $this->teacher->delete('teachers', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
            // delete teacher login data
            $this->teacher->delete('users', array('id' => $teacher->user_id,'school_id'=> $this->session->userdata('school_id')));
    ...
    public function deactivate($id)
    {
        ...
        if ($this->teacher->deactivateUser($id))
        {
            # Add success
            success('Updated status.(Deactivated');
            redirect('teacher', 'referesh');
        }
        else
        {
            # Failed
    ...
    public function activate($id)
    {
        ...
        if ($this->teacher->activateUser($id))
        {
