...
class Employee extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['teachers'] = $this->employee->get_teacher_list();
        $this->data['employees'] = $this->employee->get_employee_list();
        $this->data['roles'] = $this->employee->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        $this->data['designations'] = $this->employee->get_list('designations', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['grades'] = $this->employee->get_list('salary_grades', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_employee_data();
                $updated = $this->employee->update('employees', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $employee = $this->employee->get_single('employees', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        if (!empty($employee)) {

            // delete employee data
            $this->employee->delete('employees', array('id' => $id));
            // delete employee login data
            $this->employee->delete('users', array('id' => $employee->user_id,'school_id'=> $this->session->userdata('school_id')));
