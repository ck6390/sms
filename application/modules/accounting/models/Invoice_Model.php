...
class Invoice_Model extends MY_Model {
    ...
    public function get_invoice_list($due = null){
        ...
        $this->db->select('I.*, group_concat( DISTINCT IH.title separator ",") AS head, S.name AS student_name, S.phone AS student_phone,S.admission_no AS student_admission_no, AY.session_year, C.name AS class_name, SUM(I.net_amount) as net_amount, SUM(I.gross_amount) as gross_amount, group_concat(I.paid_status separator ",") as status, group_concat( DISTINCT I.month separator ",") AS month');
    ...
    public function get_single_invoice_by_student_month($class_id,$month,$student_id,$income_head_id,$paid_status){
     ...
     public function get_single_invoice_by_student_paid_status($student_id,$custom_invoice_id){
        $this->db->select('I.id AS inv_id');
        $this->db->from('invoices AS I');
        $this->db->where('I.school_id', $this->session->userdata('school_id'));
        $this->db->where('I.student_id', $student_id);
    ...
    public function get_annual_student_fee($student_id){
        //$this->get_annual_fee_ids($student_id);
       // die;
       // $ignore = array(17, 13);
        $school_id = $this->session->userdata('school_id');
        $this->db->select('IH.*');
        $this->db->from('income_heads AS IH'); 
        //$this->db->join('invoices AS I', 'I.income_head_id = IH.id', 'left');
        //$this->db->where('I.student_id', $student_id);
        $this->db->where('IH.fee_mode', '2');
        $this->db->where("IH.id NOT IN (SELECT income_head_id FROM invoices where school_id = $school_id and student_id = $student_id and academic_year_id = $this->academic_year_id group by income_head_id)", NULL, FALSE);
        //$this->db->where_not_in('IH.id', $ignore);
        //$this->db->where('I.school_id', $this->session->userdata('school_id'));       
        return $this->db->get()->result();
    }
    public function get_invoice_due_by_custom_invoice_id($student_id,$custom_invoice_id = null){
        $this->db->select('I.*, IH.title AS head, I.discount AS inv_discount, I.id AS inv_id , S.*, AY.session_year, C.name AS class_name','I.id AS inv_id');
        $this->db->from('invoices AS I');        
        $this->db->join('classes AS C', 'C.id = I.class_id', 'left');
        $this->db->join('students AS S', 'S.id = I.student_id', 'left');
    ...
    // fee type year_quarter wise 07-06-2019
    public function get_fee_type_by_class_year_quarter($class_id,$year_quarter){
        $this->db->select('IH.*,FA.fee_amount');
        $this->db->from('income_heads AS IH'); 
        $this->db->join('fees_amount AS FA', 'IH.id = FA.income_head_id', 'left');
        $this->db->where('IH.head_type', 'fee'); 
        $this->db->where('FA.class_id', $class_id); 
        $this->db->where('IH.school_id', $this->session->userdata('school_id'));
        $this->db->like('IH.year_quarter', '"'.$year_quarter.'"');
        $this->db->or_where('IH.head_type', 'hostel'); 
        //$this->db->or_where('IH.head_type', 'transport');        
        $this->db->group_by('IH.id'); 
        return $this->db->get()->result();  
