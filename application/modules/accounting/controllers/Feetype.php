...
class Feetype extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
         ...
         $this->data['classes'] = $this->feetype->get_list('classes', array('status'=> 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');  
    }
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_feetype_data();
                $updated = $this->feetype->update('income_heads', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
                ...
                if ($updated) {
                 ...
                 $this->data['feetype'] = $this->feetype->get_single('income_heads', array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
            }
        }
        
        if ($id) {
            $this->data['feetype'] = $this->feetype->get_single('income_heads', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
    ...
    public function get_single_feetype(){
       ...
       $this->data['feetype'] = $this->feetype->get_single('income_heads', array('id' => $feetype_id,'school_id'=> $this->session->userdata('school_id')));
    ...
    private function _prepare_feetype_validation() {
        ...
        $this->form_validation->set_rules('fee_mode', $this->lang->line('fee_mode'), 'trim|required');   
   ...
   {             
    ...
    private function _get_posted_feetype_data() {
        ...
        $items[] = 'fee_mode';
        $items[] = 'note';
        //$items[] = $data['academic_year_id'];
        $data = elements($items, $_POST);  
        $data['academic_year_id'] = $this->academic_year_id;
        $data['year_quarter'] = json_encode($this->input->post('year_quarter'));
        ...
        if($this->input->post('title') == 'Transport'){
             $data['head_type'] = 'transport';
        }elseif ($this->input->post('title') == 'Hostel') {
            $data['head_type'] = 'hostel';
        }else{
            $data['head_type'] = 'fee';
        }
    ...
    public function delete($id = null) {
        ...
        $fee_type = $this->feetype->get_single('income_heads', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        
        if ($this->feetype->delete('income_heads', array('id' => $id,'school_id'=> $this->session->userdata('school_id')))) {
            
            $this->feetype->delete('fees_amount', array('income_head_id' => $id,'school_id'=> $this->session->userdata('school_id')));            
    ...
    private function _save_fee_amount($income_head_id){
        ...
        foreach($this->input->post('class_id') as $key=>$value){
            ...
            if($amount_id){
               $exist = $this->feetype->get_single('fees_amount', array('class_id'=>$key, 'id'=>$amount_id,'school_id'=> $this->session->userdata('school_id'))); 
