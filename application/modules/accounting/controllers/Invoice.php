...
class Invoice extends MY_Controller {
                       ...
                       $duplicateInvoice = $this->invoice->get_single_invoice_by_student_month($data['class_id'],$data['month'],$data['student_id'],$data['income_head_id'],'unpaid');
                           if(isset($duplicateInvoice->inv_id) && !empty($duplicateInvoice->inv_id)){
                            ...
                            // End for Update
                             ...
                             $invoicePaymentStatus = $this->invoice->get_single_invoice_by_student_paid_status($data['student_id'], $data['custom_invoice_id']);
                                if(empty($invoicePaymentStatus->inv_id)){
                                   $insert_id = $this->invoice->insert('invoices', $data);
                                    if ($insert_id) { 
                                     ...
                                     error('Selected Student fee has been due kindly clear this, then generate Fee Receipt');
                                }
                            }
                    }
    ...
    * ********************************************************** */
    private function _prepare_invoice_validation() {
        ...
        $this->form_validation->set_rules('year_quarter', $this->lang->line('year_quarter'), 'trim|required'); 
    ...
    private function _get_posted_invoice_data() {
        ...
        foreach ($studentsList as $student) {
            if (!$this->input->post('id')) {
                ...
                $data['invoice_number'] = $this->invoice->get_invoice_number();
            }
            ...
            foreach ($months as $month) {
                ...
                foreach ($income_head_ids as $income_head_id) {
                    ...
                    //Start check for Annnual fee for single time
                    if(!empty($dataArray)){
                    foreach ($dataArray as $dataArrayData) {
                            if($income_head->fee_mode == 2 && $dataArrayData['student_id'] == $student->id && $dataArrayData['income_head_id'] == $income_head_id ){
                                $exites = 1;
                            }
                             if($income_head->fee_mode == 3 && $dataArrayData['student_id'] == $student->id && $dataArrayData['income_head_id'] == $income_head_id ){
                                $exites = 1;
                            }
                        }
                    }
                    ...
                    if($income_head->fee_mode == 2){
                        $single_invoice  = $this->invoice->get_single('invoices', array('income_head_id' => $income_head_id,'student_id' => $student->id,'school_id'=> $this->session->userdata('school_id'),'academic_year_id'=> $this->academic_year_id));
                        if($single_invoice){
                            error('Selected fee type is Annually, it will taken one time in a academic year');
                            continue;
                        }
                    }
                    //End Annual fee single time
    ...
    * ********************************************************** */
    ...
    public function get_transport_fee(){
        ...
        $annual_fee = $this->invoice->get_annual_student_fee($student_id);
    ...
    public function get_fee_amount_by_class_year_quarter(){
        
        $class_id       = $this->input->post('class_id');
        $year_quarter       = $this->input->post('year_quarter');
        $amount = 0.00;
            $fee = $this->invoice->get_fee_type_by_class_year_quarter($class_id,$year_quarter);
            if($fee){
              $amount = json_encode($fee); //$fee->fee_amount;  
            }else{
                 $amount = json_encode(array('message' => 'Please select class'));
     ...
     public function update_invoice_number(){
        ...
        foreach ($invoices as $invoice) {
           echo $invoice->custom_invoice_id.'<br>';
           $sql = "SELECT (invoice_number + 1) as inv_num FROM `invoices` ORDER BY `invoices`.`invoice_number` DESC LIMIT 1";
           $query = $this->db->query($sql, array('code', 'php'));
           $result = $query->result();
           $inv_number = (current($result)->inv_num >10) ? '0'.current($result)->inv_num : current($result)->inv_num;
           $data['invoice_number'] = $inv_number;
           $updated = $this->invoice->update('invoices', $data, array('custom_invoice_id' => $invoice->custom_invoice_id,'school_id'=> $this->session->userdata('school_id')));
           $i++.'<br>';
        }
