...
class Report_Model extends MY_Model {
    ...
    public function get_income_report($academic_year_id, $group_by, $date_from, $date_to, $created_by= null, $invoice_type = null){
       ...
       $sql = "SELECT I.*, SUM(T.amount) AS total_amount, T.payment_date, H.title AS head, AY.session_year $group_by_field 
                ...
                WHERE I.status = 1 AND I.school_id=".$this->session->userdata('school_id');
       ...
       if($created_by){
          $sql .= " AND I.created_by = '$created_by'";
       }
       if($invoice_type){
    ...
    public function get_expenditure_report($academic_year_id, $group_by, $date_from, $date_to, $created_by= null){
       ...
       if($group_by && $group_by == 'expenditure_head'){           
        ...
        $sql = "SELECT E.*, SUM(E.amount) AS total_amount, H.title AS head, AY.session_year $group_by_field 
                ...
                WHERE E.status = 1 AND E.school_id=".$this->session->userdata('school_id');
       if($date_from != '' && $date_to != ''){
    ...
    public function get_expenditure_by_date($date){
        $session_id = $this->session->userdata("school_id");
        $sql = "SELECT  SUM(E.amount) AS total_amount
                FROM expenditures AS E                
                WHERE E.date = '$date' AND E.school_id = '$session_id' GROUP BY E.date ASC";
    ...
    public function get_income_by_date($date){
      $session_id = $this->session->userdata("school_id");
        $sql = "SELECT  SUM(I.net_amount) AS total_amount
                FROM invoices AS I                
                WHERE I.date = '$date' AND I.school_id = '$session_id' GROUP BY I.date ASC";
      ...
      public function get_library_report($academic_year_id, $group_by, $date_from, $date_to){
       ...
       if($group_by && $group_by == 'daily'){           
        ...
        $sql = "SELECT BI.*, COUNT(BI.id) AS total_issue, SUM(BI.is_returned) AS total_returned, AY.session_year $group_by_field 
                ...
                WHERE BI.status = 1 AND BI.school_id=".$this->session->userdata('school_id');
    ...
    public function get_student_report($academic_year_id, $group_by){
       ...
       if($group_by && $group_by == 'gender'){           
        ...
        $sql = "SELECT S.id, COUNT(S.id) AS total, C.id as class_id, E.class_id, E.academic_year_id, AY.session_year $group_by_field 
                ...
                LEFT JOIN classes AS C ON C.id = E.class_id 
                LEFT JOIN academic_years AS AY ON AY.id = E.academic_year_id 
                WHERE S.status = 1 AND S.school_id=".$this->session->userdata('school_id');
    ...
    public function get_student_by_gender($class_id, $academic_year_id, $gender){
        ...
        $sql = "SELECT COUNT(S.id) AS total
                ...
                LEFT JOIN enrollments AS E ON E.student_id = S.id 
                LEFT JOIN classes AS C ON C.id = E.class_id 
                WHERE S.gender = '$gender' AND E.class_id = $class_id AND S.school_id = '$session_id' AND E.academic_year_id = '$academic_year_id'";
    ...
    public function get_payroll_report($academic_year_id, $group_by, $payment_to, $user_id, $month){
       ...
       if($group_by && $group_by == 'salary_type'){           
        ...
        $sql = "SELECT SP.id, SUM(SP.net_salary) AS total_amount, AY.session_year $group_by_field
                ...
                WHERE SP.status = 1 AND SP.school_id=".$this->session->userdata('school_id');
    ...
    public function get_student_invoice_report($academic_year_id, $class_id, $student_id,$date_from){
        
        /*$this->db->select('I.*,SUM(I.net_amount) AS net_amount,SUM(T.amount) AS paid_amount,IH.title AS head, ST.name AS student,C.name AS class_name, AY.session_year');
        ...
        $this->db->where('I.income_head_id !=', '19');*/

        $this->db->select('T.*,SUM(I.net_amount) AS net_amount,SUM(T.amount) AS paid_amount, ST.name AS student,ST.admission_no AS admission_no,C.name AS class_name, AY.session_year');
        $this->db->from('invoices AS I'); 
        $this->db->join('transactions AS T', 'T.invoice_id = I.id', 'left');
        $this->db->join('students AS ST', 'ST.id = I.student_id', 'left');
        $this->db->join('classes AS C', 'C.id = I.class_id', 'left');
    ...
    public function get_student_examresult_report($academic_year_id, $class_id, $section_id){
        ...
        $this->db->where('FR.school_id',$this->session->userdata('school_id'));        
