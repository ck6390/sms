...
class Report extends My_Controller {
    ...
    public function __construct() {
        ...
        $this->data['academic_years'] = $this->report->get_list('academic_years', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function balance() {
        ...
        if ($_POST) {
            ...
            if($academic_year_id){
                $this->data['academic_year'] = $this->db->get_where('academic_years', array('id'=>$academic_year_id,'school_id'=> $this->session->userdata('school_id')))->row()->session_year;
            }
    ...
    * ********************************************************** */
    public function sattendance() {
        ...
        $session = $this->report->get_single('academic_years', array('is_running' => 1,'school_id'=> $this->session->userdata('school_id')));
        ...
        if ($_POST) {
            ...
            $session = $this->report->get_single('academic_years', array('id' => $academic_year_id,'school_id'=> $this->session->userdata('school_id')));
			...
			// die();
            $this->data['academic_year'] = $this->db->get_where('academic_years', array('id'=>$academic_year_id,'school_id'=> $this->session->userdata('school_id')))->row()->session_year;
            $this->data['class_name'] = $this->db->get_where('classes', array('id'=>$class_id,'school_id'=> $this->session->userdata('school_id')))->row()->name;
            $this->data['section_name'] = $this->db->get_where('sections', array('id'=>$section_id,'school_id'=> $this->session->userdata('school_id')))->row()->name;
        ...
        $this->data['classes'] = $this->report->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function tattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['month'] = $month;
            $this->data['month_number'] = date('m', strtotime($this->data['month']));
            $this->data['teachers'] = $this->report->get_list('teachers', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
            $session = $this->report->get_single('academic_years', array('id' => $academic_year_id,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function tyattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['academic_year'] = $this->db->get_where('academic_years', array('id'=>$academic_year_id,'school_id'=> $this->session->userdata('school_id')))->row()->session_year;
            $this->data['teacher_name'] = $this->db->get_where('teachers', array('id'=>$teacher_id,'school_id'=> $this->session->userdata('school_id')))->row()->name;;
        }

        $this->data['teachers'] = $this->report->get_list('teachers', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '','', '', 'id', 'ASC');
        $this->data['days'] = 31;
        $this->data['action'] = current_url();
        $this->layout->title($this->lang->line('teacher') . ' ' . $this->lang->line('attendance') . ' ' . $this->lang->line('report') . ' | ' . SMS);
    ...
    * ********************************************************** */
    public function eattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['month'] = $month;
            $this->data['month_number'] = date('m', strtotime($this->data['month']));
            $this->data['employees'] = $this->report->get_list('employees', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function eyattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['academic_year'] = $this->db->get_where('academic_years', array('id'=>$academic_year_id,'school_id'=> $this->session->userdata('school_id')))->row()->session_year;
            $this->data['employee_name'] = $this->db->get_where('employees', array('id'=>$employee_id,'school_id'=> $this->session->userdata('school_id')))->row()->name;
        ...
        for ($i = 0; $i < $days; $i++) {           
            ...
            if(!empty($expenditure)){
                ...
                foreach($expenditure as $exp){
                    $data[$i+1]['head'] = $exp->head;                       
                    $data[$i+1]['debit'] = $exp->debit;                       
                    $data[$i+1]['credit'] = 0;                       
                    $data[$i+1]['gross'] = $exp->debit;                      
                    $data[$i+1]['tax'] = 0;                      
                    $data[$i+1]['note'] = $exp->note;                       
                    $data[$i+1]['date'] = $date; 
                }
            }
            ...
            if(!empty($income)){
                ...
                foreach($income as $inc){
                    $data[$i+1]['head'] = $inc->head;                       
                    $data[$i+1]['debit'] = 0;                       
                    $data[$i+1]['credit'] = $inc->credit;                        
                    $data[$i+1]['gross'] = $inc->credit;                      
                    $data[$i+1]['tax'] = 0;                      
                    $data[$i+1]['note'] = $inc->note;                       
                    $data[$i+1]['date'] = $date; 
                }
            }
    ...
    * ********************************************************** */
    public function sinvoice() {
        ...
        if ($_POST) {
            ...
            $this->data['sbalance'] = $this->report->get_student_invoice_report($academic_year_id, $class_id, $student_id,$date_from);
         }
    ...
    * ********************************************************** */
    public function feecollection() {
        ...
        $this->data['classes'] = $this->report->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['fee_types'] = $this->report->get_list('income_heads', array('status' => 1, 'head_type !='=> 'income','school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function examresult() {
        ...
        if ($_POST) {
            ...
            $this->data['class'] = $this->db->get_where('classes', array('id'=>$class_id,'school_id'=> $this->session->userdata('school_id')))->row()->name;
    ...
    public function accountant1() {
        ...
        if ($_POST) {
            ...
            if ($group_by == 'daily') {
                ...
                $this->data['expenditure'] = $this->report->get_expenditure_byaccountant_report($academic_year_id, $group_by, $date_from, $date_to);
                $this->data['income'] = $this->report->get_income_byaccountant_report($academic_year_id, $group_by, $date_from, $date_to);
                $this->data['fee'] = $this->report->get_income_byaccountant_report($academic_year_id, $group_by, $date_from, $date_to,'fee');

                // Get Users
                $this->db->select('E.*, U.email, U.role_id as urole_id');
                $this->db->from('employees AS E');
                $this->db->join('users AS U', 'U.id = E.user_id', 'left');
                $this->db->where('E.school_id', $this->session->userdata('school_id'));
                $this->db->where('U.role_id = 6');
                $accountants = $this->db->get()->result();
                //End Get users
                foreach ($this->data['income'] as $incomes) {
                    $income[$incomes->created_by] = $incomes->total_amount;
                }
                foreach ($this->data['expenditure'] as $expenditures) {
                    $expenditure[$expenditures->created_by] = $expenditures->total_amount;
                }
                foreach ($this->data['fee'] as $fees) {
                    $fee[$fees->created_by] = $fees->total_amount;
                }
                foreach ($accountants as $accountKey => $accountant) {
                    $list[$accountant->user_id]['name'] = $accountant->name;
                    $list[$accountant->user_id]['income'] = @$income[$accountant->user_id];
                    $list[$accountant->user_id]['fee'] = @$fee[$accountant->user_id];
                    $list[$accountant->user_id]['expenditure'] = @$expenditure[$accountant->user_id];
                }
                $this->data['list'] = $list;
            }
        }
        ...
        $this->layout->title($this->lang->line('balance') . ' ' . $this->lang->line('report') . ' | ' . SMS);
        $this->layout->view('balance/accountant', $this->data);
    }
    ...
    public function accountant() {
        ...
        if ($_POST) {
            ...
            if ($group_by == 'daily') {
                ...
                $this->data['expenditure'] = $this->report->get_expenditure_report($academic_year_id, $group_by, $date_from, $date_to, $user_id);
                $this->data['income'] = $this->report->get_income_report($academic_year_id, $group_by, $date_from, $date_to ,$user_id, 'income');
                $this->data['fee'] = $this->report->get_income_report($academic_year_id, $group_by, $date_from, $date_to ,$user_id, 'fee');
                $this->data['balance'] = $this->_combine_data($this->data['expenditure'], $this->data['income'],$this->data['fee'] );
            }
        }
        // Get Users
            $this->db->select('E.*, U.email, U.role_id as urole_id');
            ...
            $this->db->where('E.school_id', $this->session->userdata('school_id'));
            $this->db->where('U.role_id = 6');
            $this->data['accountants'] = $this->db->get()->result();
        //End Get users
        $this->data['action'] = current_url();
        $this->layout->title($this->lang->line('accountant') . ' ' . $this->lang->line('report') . ' | ' . SMS);
        $this->layout->view('balance/accountant', $this->data);
    }
    private function _combine_data($expenditure, $income, $fee) {
        $data = array();
        foreach ($expenditure as $obj) {
            $data[$obj->group_by_field]['expenditure'] = $obj->total_amount;
            $data[$obj->group_by_field]['group_by_field'] = $obj->group_by_field;
            ...
            if (empty($data[$obj->group_by_field]['fee'])) {
                $data[$obj->group_by_field]['fee'] = 0;
            }
        }
        foreach ($income as $obj) {
            $data[$obj->group_by_field]['income'] = $obj->total_amount;
        ...
        foreach ($fee as $obj) {
            $data[$obj->group_by_field]['fee'] = $obj->total_amount;
            $data[$obj->group_by_field]['group_by_field'] = $obj->group_by_field;
