...
class Route extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['routes'] = $this->route->get_list('routes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_route_data();
                $updated = $this->route->update('routes', $data, array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
                ...
                $this->data['post'] = $_POST;
                $this->data['route'] = $this->route->get_single('routes', array('id' => $this->input->post('id'),'school_id'=> $this->session->userdata('school_id')));
            }
        }

        if ($id) {
            $this->data['route'] = $this->route->get_single('routes', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        ...
        $this->data['route_stops'] = $this->route->get_list('route_stops', array('status' => 1,'route_id'=>$id,'school_id'=> $this->session->userdata('school_id')), '','', '','','id','ASC');
    ...
    * ********************************************************** */
    ...
    public function get_single_route(){
       ...
       $this->data['route'] = $this->route->get_single('routes', array('id' => $route_id,'school_id'=> $this->session->userdata('school_id')));
       $this->data['route_stops'] = $this->route->get_list('route_stops', array('status' => 1,'route_id'=>$route_id,'school_id'=> $this->session->userdata('school_id')), '','', '','','id','ASC');
    ...
    * ********************************************************** */
    private function _update_vehicle_status($id = null) {
        ...
        if ($id) {
            $route = $this->route->get_single('routes', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
            $ids = explode(',', $route->vehicle_ids);
            if (!empty($ids)) {
                foreach ($ids as $key => $value) {
                    $this->route->update('vehicles', array('is_allocated' => 0), array('id' => $value,'school_id'=> $this->session->userdata('school_id')));
                }
            }
        }
        ...
        if ($this->input->post('vehicle_ids')) {
            foreach ($this->input->post('vehicle_ids') as $key => $value) {
                $this->route->update('vehicles', array('is_allocated' => 1), array('id' => $value,'school_id'=> $this->session->userdata('school_id')));
            }
        }
    }
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        if ($this->route->delete('routes', array('id' => $id))) {
            ...
            // delete route stop
            $this->route->delete('route_stops', array('route_id' => $id,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    private function _save_stop($route_id){
        ...
        foreach($this->input->post('stop_name') as $key=>$value){
            ...
            if($value){
                ...
                if($stop_id){
                   $exist = $this->route->get_single('route_stops', array('route_id'=>$route_id, 'id'=>$stop_id,'school_id'=> $this->session->userdata('school_id')));
                }         
                ...
                if ($this->input->post('id') && $exist) {                
                    ...
                    $data['modified_at'] = date('Y-m-d H:i:s');
                    $data['modified_by'] = logged_in_user_id();                
                    $this->route->update('route_stops', $data, array('id'=>$exist->id,'school_id'=> $this->session->userdata('school_id')));
    ...
    public function remove_stop(){
        ...
        echo $this->route->delete('route_stops', array('id' => $stop_id,'school_id'=> $this->session->userdata('school_id')));
    }
