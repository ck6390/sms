...
class Member extends MY_Controller {
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['routes'] = $this->member->get_list('routes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')));
        $this->data['add'] = TRUE;
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $member = $this->member->get_single('transport_members', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
        if ($this->member->delete('transport_members', array('id' => $id,'school_id'=> $this->session->userdata('school_id')))) {

            $this->member->update('students', array('is_transport_member' => 0), array('user_id' => $member->user_id,'school_id'=> $this->session->userdata('school_id')));
            
            $student = $this->member->get_single('students', array('user_id' => $member->user_id,'school_id'=> $this->session->userdata('school_id')));
    ...
    * ********************************************************** */
    public function add_to_transport() {
        ...
        if ($user_id) {
            ...
            $member = $this->member->get_single('transport_members', array('user_id' => $user_id,'school_id'=> $this->session->userdata('school_id')));
            if (empty($member)) {
                ...
                $this->member->update('students', array('is_transport_member' => 1), array('user_id' => $user_id,'school_id'=> $this->session->userdata('school_id')));
