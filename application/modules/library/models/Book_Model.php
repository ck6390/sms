...
class Book_Model extends MY_Model {
    ...
    public function get_book_issued_list() {
        ...
        $this->db->where('BI.school_id', $this->session->userdata('school_id'));
        if($user_id){
    ...
    public function update_qty($book_id, $type = null){
        ...
        if($type == 'issue'){
            $sql = "UPDATE books SET qty = qty - 1 WHERE id = $book_id AND school_id =".$this->session->userdata('school_id');
        }else{
            $sql = "UPDATE books SET qty = qty + 1 WHERE id = $book_id AND school_id =". $this->session->userdata('school_id');
        }
