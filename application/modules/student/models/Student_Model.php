...
class Student_Model extends MY_Model {
    ...
    public function get_student_list($class_id = null, $end_year = null){
        ...
        $this->db->select('S.*, E.roll_no, U.email, U.role_id,U.unique_id,  C.name AS class_name, SE.name AS section, AY.session_year, S.signature');
    ...
    public function get_single_student($id, $adv = null){
        ...
        $this->db->select('S.*, D.amount, D.title AS discount_title, G.name as guardian, E.roll_no, E.class_id, E.section_id, E.academic_year_id,E.is_transport,E.is_hostel, U.email, U.role_id,U.unique_id, R.name AS role,  C.name AS class_name, SE.name AS section, S.signature');
        $this->db->from('enrollments AS E');
        $this->db->join('students AS S', 'S.id = E.student_id', 'left');
        $this->db->join('users AS U', 'U.id = S.user_id', 'left');
    ...
    public  function deactivateUser($id)
    {
        ...
        $query = $this->db->update('students', $status, array('id'=>$id));
   ...
   public function passwordupdate($userid,$school_id,$password)
   {
        $pass= array(
            'password'=>$password
         );
