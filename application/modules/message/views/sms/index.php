                                ...
                                <tbody>   
                                    <?php $count = 1; if(isset($texts) && !empty($texts)){ ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if($role_id == 100000){echo "Others";}else{ echo $obj->receiver_type;} ?></td>
                               ...
                               <?php echo form_open_multipart(site_url('message/text/add'), array('name' => 'add', 'id' => 'add', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                <div class="item form-group"> 
                                    <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                        <input type="radio" name="option" value="1" checked required>&nbsp;&nbsp;<strong>Internal</strong><?php echo str_repeat("&nbsp;", 10);?>
                                        <input type="radio" name="option"  value="0" required>&nbsp;&nbsp;<strong>External</strong>
                                        <div class="help-block"><?php echo form_error('option'); ?></div>
                                    </div>
                                </div>
                              
                                 <div class="item form-group" id="external" hidden="true"> 
                                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="role_id"><?php echo $this->lang->line('receiver'); ?> <span class="required">*</span></label>
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="number" name="send_to" class="form-control col-md-12 col-xs-12" >
                                        <div class="help-block"><?php echo form_error('send_to'); ?></div>
                                        <input type="hidden" name="recivers_to" value="100000">
                                    </div>
                                </div>
                                <div id="internal">
                                <div class="item form-group" > 
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select  class="form-control col-md-12 col-xs-12"  name="role_id"  id="role_id" required="required" onchange="get_user_by_role(this.value, '');">
                                            ...
                                            <option value="<?php echo $obj->id.",".$obj->name; ?>" <?php if(isset($message) && $message->role_id == $obj->id ){ echo 'selected="selected"';} ?> ><?php echo $obj->name; ?></option>
                                            <?php } ?>                                            
                                        </select>
...
/*function ShowHideDiv() {
        ...
        var detext = document.getElementById("detext");
        dvtext.style.display = chkYes.checked ? "block" : "none";
        detext.style.display = chkNo.checked ? "block" : "none";
    }*/
    ...
    $("input[name='option']").change(function(){
            if(this.value == 1){
                $("#internal").show();
                $("#external").hide();
                $("input[name='send_to']").prop('disabled', true);
                $("input[name='recivers']").prop('disabled', true);
                $("input[name='role_id']").prop('disabled', false);
                $("input[name='class_id']").prop('disabled', false);
                $("input[name='receiver_id']").prop('disabled', false);
            }else{
                $("#external").show();
                $("#internal").hide();
                $("input[name='role_id']").prop('disabled', true);
                $("input[name='class_id']").prop('disabled', true);
                $("input[name='receiver_id']").prop('disabled', true);
                 $("input[name='send_to']").prop('disabled', false);
                $("input[name='recivers']").prop('disabled', false);
            }
        });
