                            ...
                            <?php echo form_open_multipart(site_url('message/reply/'.$message->message_id), array('name' => 'compose', 'id' => 'compose', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                ...
                                <div class="box-body no-padding">
                                  <div class="mailbox-read-info">
                                    ...
                                    <h5> <span><?php echo $this->lang->line('sender'); ?>:
                                        <?php if($message->receiver_id == $message->owner_id){ ?>
                                           <?php if($this->session->userdata('user_type') != "School"){
												...
												$this->db->where('S.user_id', $message->sender_id);
												$user = $this->db->get()->row();
												echo $user->school_name;
											}else{
												$user = get_user_by_id($message->sender_id); echo $user->name;
											} ?>
                                        ...
                                        <?php } ?>
                                      <span class="mailbox-read-time pull-right"><?php echo date($this->config->item('date_format').' H:i:s a', strtotime($message->created_at)); ?></span>
									</span></h5>
                                  </div>
                                  ...
                                  <?php if(isset($replies) && !empty($replies)){ ?>
                                    ...
                                    <h5>                                      
                                        ...
                                        <span class="mailbox-read-time pull-right"><?php echo date($this->config->item('date_format'). ' H:i:s a', strtotime($obj->created_at)); ?></span>
                                    </h5>     
                                      <div class="mailbox-read-message">
