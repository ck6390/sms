...
class Message extends MY_Controller {
    ...
    public function compose($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->input->post('message_id')) {
                ...
                $this->message->update('messages', $data, array('id' => $this->input->post('message_id'),'school_id'=> $this->session->userdata('school_id')));
            } else {
                $data['status'] = 1;
			...
			$relation_data['school_id'] = $this->session->userdata('school_id');
        ...
        $this->data['classes'] = $this->message->get_list('classes', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['roles'] = $this->message->get_list('roles', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function view($id = null) {
        ...
        if ($id) {
            $this->data['message'] = $this->message->get_single_message($id);
            $this->data['replies'] = $this->message->get_list('replies', array('message_id' => $id,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $exist = $this->message->get_list('message_relationships', array('status' => 1, 'message_id' => $id, 'owner_id !=' => logged_in_user_id(),'school_id'=> $this->session->userdata('school_id')));

        if (!empty($exist)) {
            $data = array('status' => 0);
            $condition = array('message_id' => $id, 'owner_id' => logged_in_user_id(),'school_id'=> $this->session->userdata('school_id'));
            $this->message->update('message_relationships', $data, $condition);
            success($this->lang->line('delete_success'));
        } else {
            $this->message->delete('message_relationships', array('message_id' => $id,'school_id'=> $this->session->userdata('school_id')));
            $this->message->delete('messages', array('id' => $id,'school_id'=> $this->session->userdata('school_id')));
            $this->message->delete('replies', array('message_id' => $id,'school_id'=> $this->session->userdata('school_id')));
            success($this->lang->line('delete_success'));
        }
    ...
    * ********************************************************** */
    public function set_fvourite_status() {
        ...
        $condition = array('message_id' => $message_id, 'owner_id' => logged_in_user_id(),'school_id'=> $this->session->userdata('school_id'));
        $this->message->update('message_relationships', $data, $condition);
        echo TRUE;
    }
    ...
    public function set_trash_status() {
        ...
        if (!empty($message_ids)) {
            foreach ($message_ids as $value) {
                $condition = array('message_id' => $value, 'owner_id' => logged_in_user_id(),'school_id'=> $this->session->userdata('school_id'));
                $this->message->update('message_relationships', $data, $condition);
            }
        }
