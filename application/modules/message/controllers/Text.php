...
class Text extends MY_Controller {
   ...
   public function _prepare_sms_validation() {
        ...
        $this->form_validation->set_rules('option', $this->lang->line('option'), 'trim|required');
        $this->form_validation->set_rules('send_to', $this->lang->line('receiver'), 'trim|required');
    ...
    * ********************************************************** */
    private function _get_posted_sms_data() {
        ...
        if ($this->input->post('option') == '0'){
            $data['role_id']= $this->input->post('recivers_to');
            $data['receivers']= $this->input->post('send_to');
        }
    ...
    * ********************************************************** */
    private function _send_sms($data) {
        ...
        $receivers = '';
        if($this->input->post('option') == '0'){
       $users = $this->sms->get_user_list($data['role_id'], $this->input->post('send_to'), $this->input->post('class_id'));
       //var_dump($users);die();
       }else{
    ...
    * ********************************************************** */
    public function _send($sms_gateway, $phone, $message) {
        //var_dump($sms_gateway, $phone, $message);
