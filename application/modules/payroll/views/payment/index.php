                                ...
                                <tbody>   
                                    <?php $count = 1; if(isset($payments) && !empty($payments)){ ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo date('M, Y', strtotime($obj->salary_month)); ?></td>
                                            ...
                                            <td>
                                                ...
                                                <?php if(has_permission(VIEW, 'payroll', 'payment')){ ?>
                                                    ...
                                                    <a href="<?php echo site_url('payroll/payment/view/'.$obj->id.'/'.$payment_to); ?>" class="btn btn-success btn-xs"><i class="fa fa-eye"></i> <?php echo $this->lang->line('view'); ?> </a>
                                                <?php } ?>
                            ...
                            <div  class="tab-pane fade in <?php if(isset($add)){ echo 'active'; }?>" id="tab_add_payment">
                                <div class="x_content"> 
                                   <?php echo form_open(site_url('payroll/payment/add'), array('name' => 'add', 'id' => 'add', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                    ...
                                    <div class="row display fn_cheque" style="display: none;">
                                        <div class="col-md-3 col-sm-3 col-xs-12">
                                            <div class="item form-group">
                        ...
                        <div class="tab-pane fade in active" id="tab_edit_payment">
                            <div class="x_content"> 
                               <?php echo form_open(site_url('payroll/payment/edit/'.$edit_payment->id), array('name' => 'edit', 'id' => 'edit', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                ...
                                <div class="row fn_cheque" <?php if(isset($edit_payment) && $edit_payment->payment_method == 'cash'){ ?> style="display: none;" <?php  } ?>>
                                    <div class="col-md-3 col-sm-3 col-xs-12">
                                        <div class="item form-group">
                                            <label for="bank_name"><?php echo $this->lang->line('bank'); ?> <?php echo $this->lang->line('name'); ?><span class="required">*</span> </label>
...
<!-- datatable with buttons -->
 <script type="text/javascript">
    ...
    function calculate_salary(date){
        var user_id = $('#user_id').val();
        var basic_salary = $('#add_basic_salary').val();
        var payment_to = $('#payment_to').val();
       $.ajax({       
            type   : "POST",
            url    : "<?php echo site_url('ajax/get_salary'); ?>",
            data   : { date : date, user_id : user_id,basic_salary : basic_salary,payment_to : payment_to},               
            async  : false,
            dataType: 'json',
            success: function(response){
               if(response.status)
               {
                   $('#add_gross_salary').val(response.netSalary); 
                   $('#add_net_salary').val(response.netSalary); 
                   $('#add_total_deduction').val(response.total_deduction); 
                   $('#edit_gross_salary').val(response.netSalary); 
                   $('#edit_net_salary').val(response.netSalary); 
                   $('#edit_total_deduction').val(response.total_deduction); 
               }else{
                    alert(response.message);
                    $('#add_salary_month').val('');
                    $('#add_gross_salary').val(''); 
                    $('#add_net_salary').val('');
                    $('#edit_gross_salary').val(''); 
                    $('#edit_net_salary').val(''); 
                    $('#edit_total_deduction').val('');
                    return false;
               }
            }
        }); 
