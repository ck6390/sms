  ...
  class Auth extends CI_Controller {
    ...
    public function __construct() {
      ...
      public function login() {
        if ($_POST) { 
          ...
          if ($this->form_validation->run())
          {
            ...
            $res = str_split($login_id[3]);               
            if($res[0] == "/"){                   
              $data['unique_id'] = $this->input->post('email');  
              $check_user_type = $this->auth->get_single('users', $data); 
              if(!empty($check_user_type)){
                $is_verify = $this->auth->get_single('licences', array('s_id'=>$check_user_type->school_id ));    
                if($is_verify){
                  if($is_verify->is_verify == 0){
                    $this->session->set_flashdata('error', 'Your licences not verified.Please verify now');
                    redirect('verify', 'referesh');
                  }elseif ($is_verify->active == 0) {
                    $this->session->set_flashdata('error', 'Your licences not approved.Please contact to admin');
                    redirect('verify', 'referesh');
                  }elseif($is_verify->status == 0){
                    $this->session->set_flashdata('error', 'Your account has been deleted.');
                    redirect('verify', 'referesh');
                  }elseif(strtotime(date('d-m-Y')) >= strtotime($is_verify->expiry_date)){
                    $this->session->set_flashdata('error', 'Your licences has been expired.');
                ...
                $this->session->set_flashdata('error', 'Login user id does not match');
              ...
              $check_user_type = $this->auth->get_single('users', $data);      
              if(!empty($check_user_type)){           
                if($check_user_type->user_type=="School"){

                  $is_verify = $this->auth->get_single('licences', array('email'=>$data['email']));            
                  if($is_verify){
                    if($is_verify->is_verify == 0){
                      ...
                      $this->session->set_flashdata('error', 'Your licences has been expired.Please contact to admin');
                      redirect('login', 'referesh');
                    }
                  } 
                  else{
              ...
              $this->session->set_flashdata('error', 'Loginwaza user id does not match');
              redirect('welcome', 'referesh');
            }
          }
     ...
     // var_dump($login);
          if (!empty($login)) {
        $academic_year = $this->db->get_where('academic_years', array('is_running'=>1, 'school_id'=> $login->school_id))->row();
        if($academic_year){
        $this->academic_year_id = $academic_year->id;
        }
            ...
            if ($login->is_deleted=="0") {
             $this->session->set_flashdata('error', 'This account has been deleted.');
             redirect('welcome');
           }
      ...
      //var_dump($profile);
      //die();
          } elseif ($login->user_type == "Guardian") {
            $profile = $this->auth->get_single('guardians', array('user_id' => $login->id));
          } elseif ($login->user_type == "Teacher") {
     ...
     // var_dump($profile);
          $this->session->set_userdata('u_id', $login->id);
          ...
          $setting = $this->auth->get_single('settings', array('status' =>1,'id' => $login->school_id));               
          if (isset($setting->school_name)) {
            $this->session->set_userdata('school_name', $setting->school_name);
          }
           if (isset($setting->admission_start)) {
            $this->session->set_userdata('admission_start', $setting->admission_start);
          }
           if (isset($setting->admission_end)) {
            $this->session->set_userdata('admission_end', $setting->admission_end);
          }
          ...
          if (isset($setting->school_logo)) {
            $this->session->set_userdata('school_logo', $setting->school_logo);
          }
          if (isset($setting->su_id)) {
            $this->session->set_userdata('su_id', $setting->su_id);
          }
          ...
          if($this->session->userdata('user_type') == "School"){ 
			if (isset($setting->school_name)) {
			 ...
			 if (isset($setting->school_phone)) {
			$this->session->set_userdata('school_phone', $setting->school_phone);
			}
			if (isset($is_verify->expiry_date)) {
			$this->session->set_userdata('expiry_date', $is_verify->expiry_date);
			} 
		  }
        else{
          ...
          if (isset($profile->phone)) {
            ...
            //// ------------Alloted Module -------------//
        $alloted_module = $this->auth->get_single('operation_menu', array('status' =>1,'school_id' => $setting->su_id));
        if (isset($alloted_module->menu_id)) {
          $this->session->set_userdata('alloted_menu_id', $alloted_module->menu_id);                       
        } 
            ///------------All Menu --------------//
        $all_menu = $this->auth->get_list('menu', array('status' => 1,'active'=>1), '', '', '', 'id', 'ASC');
        if (isset($all_menu)) {
          $this->session->set_userdata('all_menu', $all_menu);                       
        } 
      ...
      * ********************************************************** */
      ...
      public function resetpass() {
        if ($_POST) {
          ...
          $this->form_validation->set_rules('password', $this->lang->line('password'), 'rim|required|min_length[5]|max_length[12]');
