...
class Dashboard extends MY_Controller {
    ...
    public function index() {
      ...
      $this->data['year_session'] = $this->dashboard->get_single('academic_years', array('is_running' => 1));
        ...
        $this->data['news'] = $this->dashboard->get_list('news', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '5', '', 'id', 'ASC');
        $this->data['notices'] = $this->dashboard->get_list('notices', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '5', '', 'id', 'ASC');
        $this->data['events'] = $this->dashboard->get_list('events', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['holidays'] = $this->dashboard->get_list('holidays', array('status' => 1,'school_id'=> $this->session->userdata('school_id')), '', '', '', 'id', 'ASC');
        $this->data['theme'] = $this->dashboard->get_single('themes', array('status' => 1, 'is_active' => 1,'school_id'=> $this->session->userdata('school_id')));
      ...
      //  die();
        ...
        $this->data['total_dues'] = $this->dashboard->get_total_dues();
        ...
        // 05-07-2019
        $collected_data = $this->dashboard->get_collected_amount_month_wise();

        $collected_amount= array();
        if($collected_data){
          $i=0;
            foreach ($collected_data as $collected_data) {
               $collected_amount[$i]['month'] = $collected_data->displaydate;
               $collected_amount[$i]['amount'] = $collected_data->amount;
               $i++;
            }

        }
        $this->data['collected_amount'] = $collected_amount;
        

        $income_headwise = $this->dashboard->get_amount_by_incomehead();
        $income_head_collection= array();
        if($collected_data){
          $i=0;
            foreach ($income_headwise as $income_data) {
               $income_head_collection[$i]['title'] = ucfirst(strtolower($income_data->title));
               $income_head_collection[$i]['amount'] = $income_data->amount;
               $i++;
            }

        }
        $this->data['income_head_collection'] = $income_head_collection;
        // 05-07-2019
